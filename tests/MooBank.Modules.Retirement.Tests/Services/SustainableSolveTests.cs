...
public class SustainableSolveTests
{
    ...
    {
        // Act
        var solved = _engine.Calculate(Plan(), _today, _rates, StrategyRates, MinimumDrawdownRates.None).Summary.SustainableIncomeInTodaysDollars;

        // Assert
        Assert.True(solved > 0m);

        var atSolved = _engine.Calculate(Plan(solved), _today, _rates, StrategyRates, MinimumDrawdownRates.None).Summary;
        Assert.Null(atSolved.MoneyRunsOutYear);

        // A hundred pounds more than the solve claims is beyond the rounding it allows for.
        var justOver = _engine.Calculate(Plan(solved + 500m), _today, _rates, StrategyRates, MinimumDrawdownRates.None).Summary;
    ...
    {
        var withPension = _engine.Calculate(Plan(), _today, _rates, StrategyRates, MinimumDrawdownRates.None).Summary.SustainableIncomeInTodaysDollars;
        var without = _engine.Calculate(Plan(), _today, AgePensionRates.None, StrategyRates, MinimumDrawdownRates.None).Summary.SustainableIncomeInTodaysDollars;
    ...
    {
        var to85 = _engine.Calculate(Plan(lifeExpectancy: 85), _today, _rates, StrategyRates, MinimumDrawdownRates.None).Summary.SustainableIncomeInTodaysDollars;
        var to95 = _engine.Calculate(Plan(lifeExpectancy: 95), _today, _rates, StrategyRates, MinimumDrawdownRates.None).Summary.SustainableIncomeInTodaysDollars;
    ...
    public void Solve_WithAMemberExcluded_IsForTheRemainingHousehold()
    {
        ...
        // Act
        var household = _engine.Calculate(plan, _today, _rates, StrategyRates, MinimumDrawdownRates.None).Summary.SustainableIncomeInTodaysDollars;
        var alone = _engine.Calculate(plan, _today, _rates, StrategyRates, MinimumDrawdownRates.None, new ProjectionOverrides { ExcludedMemberIds = [spouse.Id] })
    ...
    public void Solve_NoMembers_IsNought()
    {
        ...
        Assert.Equal(0m, _engine.Calculate(plan, _today, _rates, StrategyRates, MinimumDrawdownRates.None).Summary.SustainableIncomeInTodaysDollars);
    }
}
