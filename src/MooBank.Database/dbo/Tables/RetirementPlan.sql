...
(
    ...
    [TargetRetirementIncome] DECIMAL(18,2) NOT NULL CONSTRAINT DF_RetirementPlan_TargetRetirementIncome DEFAULT 0,
    [PreRetirementSwitchYears] INT NOT NULL CONSTRAINT DF_RetirementPlan_PreRetirementSwitchYears DEFAULT 2,
    [CashReturnRate] DECIMAL(6,4) NOT NULL CONSTRAINT DF_RetirementPlan_CashReturnRate DEFAULT 0.03,
    ...
    CONSTRAINT [CK_RetirementPlan_TargetRetirementIncome] CHECK ([TargetRetirementIncome] >= 0),
    -- Nought is meaningful: it turns the de-risking glide off and leaves a member in their chosen
    -- strategy right through retirement.
    CONSTRAINT [CK_RetirementPlan_PreRetirementSwitchYears] CHECK ([PreRetirementSwitchYears] BETWEEN 0 AND 40),
    CONSTRAINT [CK_RetirementPlan_CashReturnRate] CHECK ([CashReturnRate] BETWEEN -1 AND 1)
)
GO
