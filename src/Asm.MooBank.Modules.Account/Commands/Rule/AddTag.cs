...
internal class AddTagHandler(IAccountRepository accountRepository, ITransactionTagRepository tagRepository, IUnitOfWork unitOfWork, AccountHolder accountHolder, ISecurity security) : CommandHandlerBase(unitOfWork, accountHolder, security), ICommandHandler<AddTag, Models.Rule>
{
    private readonly IAccountRepository _accountRepository = accountRepository;
    private readonly ITransactionTagRepository _tagsRepository = tagRepository;
    ...
    public async ValueTask<Models.Rule> Handle(AddTag request, CancellationToken cancellationToken)
    {
        ...
        if (rule.Tags.Any(t => t.Id == request.TagId)) return rule;
