namespace Asm.MooBank.Security;
public static class ClaimTypes
{
    public const string UserId = "http://schemas.microsoft.com/identity/claims/objectidentifier";
    public const string FamilyId = "http://moobank.mclachlan.family/claims/familyid";
    public const string AccountId = "http://moobank.mclachlan.family/claims/accountid";
    public const string PrimaryAccountId = "http://moobank.mclachlan.family/claims/primaryaccountid";
}
