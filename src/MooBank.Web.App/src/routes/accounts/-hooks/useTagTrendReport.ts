...
export const useTagTrendReport = (accountId: string, start: Date, end: Date, reportType: transactionTypeFilter, tagId: number, settings: TrendReportSettings) =>
    useQuery({
        ...tagTrendReportOptions({ path: { accountId, start: start ? formatISODate(start) : "", end: end ? formatISODate(end) : "", reportType: reportType.toLowerCase() as any, tagId }, query: { ApplySmoothing: settings.applySmoothing } }),
