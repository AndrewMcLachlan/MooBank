...
internal class GetForVirtualHandler(IQueryable<Domain.Entities.Instrument.Instrument> accounts) : IQueryHandler<GetForVirtual, IEnumerable<RecurringTransaction>>
{
    ...
    {
        var account = (await accounts.Include(a => a.VirtualInstruments).ThenInclude(a => a.RecurringTransactions).SingleOrDefaultAsync(a => a.Id == query.AccountId, cancellationToken)) ?? throw new NotFoundException();
