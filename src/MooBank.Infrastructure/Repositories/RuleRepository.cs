...
{
    // Rules run in the background without a user context, so the Family filter is lifted here.
    ...
    {
        var entity = await Entities.Include(r => r.Tags).IgnoreQueryFilters(["Family"]).Where(r => r.Id == id && r.InstrumentId == accountId).SingleOrDefaultAsync(cancellationToken) ?? throw new NotFoundException();
        Context.Remove(entity);
    }

    public async Task<Rule> Get(Guid accountId, int id, CancellationToken cancellationToken = default)
    {
        var rule = await Entities.Include(t => t.Tags).IgnoreQueryFilters(["Family"]).Where(t => t.InstrumentId == accountId && t.Id == id).SingleOrDefaultAsync(cancellationToken);
    ...
    public async Task<IEnumerable<Rule>> GetForInstrument(Guid accountId, CancellationToken cancellationToken = default) =>
        await Entities.Include(t => t.Tags).IgnoreQueryFilters(["Family"]).Where(t => t.InstrumentId == accountId).OrderBy(t => t.Contains).ToListAsync(cancellationToken);
