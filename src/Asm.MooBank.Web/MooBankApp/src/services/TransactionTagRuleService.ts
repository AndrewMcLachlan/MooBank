...
export const useRules = (accountId: string) => useApiGet<Models.Rules>([transactionRulesKey, accountId], `api/accounts/${accountId}/transaction/tag/rules`);
...
export const useAddTransactionTagRuleTag = () => {
    ...
    return useApiDatalessPut<Models.Rule, TransactionTagRuleVariables>((variables) => `api/accounts/${variables.accountId}/transaction/tag/rules/${variables.ruleId}/tag/${variables.tag.id}`, {
        onMutate: (variables) => {
            const rules = queryClient.getQueryData<Models.Rules>([transactionRulesKey, variables.accountId]);
            if (!rules) return;
            const data = rules.rules.find(t => t.id === variables.ruleId);
            if (!data) return;
            data.tags.push(variables.tag);
            queryClient.setQueryData<Models.Rules>([transactionRulesKey, variables.accountId], rules);
        },
    });
}
...
export const useCreateRule = () => {
    ...
    return useApiPost<Models.Rule, { accountId: string }, Models.Rule>((variables) => `api/accounts/${variables.accountId}/transaction/tag/rules`, {
        onSuccess: (data, variables) => {
            const allRules = queryClient.getQueryData<Models.Rules>([transactionRulesKey, variables[0].accountId]);
            if (!allRules) {
            ...
            queryClient.setQueryData<Models.Rules>([transactionRulesKey, variables[0].accountId], allRules);
        }
    });
}
...
export const useUpdateRule = () => {
    ...
    return useApiPatch<Models.Rule, { accountId: string, id: number }, Models.Rule>((variables) => `api/accounts/${variables.accountId}/transaction/tag/rules/${variables.id}`, {

        onMutate: ([variables, data]) => {
            const allRules = queryClient.getQueryData<Models.Rules>([transactionRulesKey, variables.accountId]);
            if (!allRules) {
                console.warn("Query Cache is missing Transaction Rules");
                return;
            }
            ...
            allRules.rules = allRules.rules.sort((t1, t2) => t1.contains.localeCompare(t2.contains));
            queryClient.setQueryData<Models.Rules>([transactionRulesKey, variables.accountId], allRules);
        },
