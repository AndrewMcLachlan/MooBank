    ...
    IUnitOfWork unitOfWork) : ICommandHandler<UpdatePlan, Models.RetirementPlan>
{
    ...
    private static void ReconcileMembers(Domain.Entities.Retirement.RetirementPlan entity, IEnumerable<Models.RetirementPlanMember> members)
    {
        ...
        foreach (var member in supplied)
        {
            ...
            {
                entity.AddMember(member.UserId!.Value, new Asm.MooBank.Domain.Entities.Retirement.RetirementMemberDetails(
                member.CurrentAge, member.CurrentIncome, member.SalarySacrifice, member.SalaryGrowthRate,
                member.RetirementAge, member.GrowthStrategy, member.CustomReturnRate,
                member.RetirementGrowthStrategy, member.RetirementCustomReturnRate,
                member.AnnualFees, member.InsurancePremium), member.InstrumentIds);
                continue;
            }
            ...
            existing.Update(new Asm.MooBank.Domain.Entities.Retirement.RetirementMemberDetails(
                member.CurrentAge, member.CurrentIncome, member.SalarySacrifice, member.SalaryGrowthRate,
                member.RetirementAge, member.GrowthStrategy, member.CustomReturnRate,
                member.RetirementGrowthStrategy, member.RetirementCustomReturnRate,
                member.AnnualFees, member.InsurancePremium));
