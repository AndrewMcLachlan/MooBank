...
internal class SearchHandler : IQueryHandler<Search, IEnumerable<Models.Transaction>>
{
    ...
    public Task<IEnumerable<Models.Transaction>> Handle(Search request, CancellationToken cancellationToken)
    {
        ...
        return _transactions.IncludeAll().Where<Transaction>(t => t.AccountId == request.AccountId && t.Offsets == null && t.TransactionTime >= startTime && t.TransactionType == request.TransactionType && t.TransactionTags.Any(tt => request.TagIds.Contains(tt.Id))).ToModelAsync(cancellationToken: cancellationToken);
    }
}
