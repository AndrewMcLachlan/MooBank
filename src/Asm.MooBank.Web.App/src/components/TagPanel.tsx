...
export const TagPanel: React.FC<TagPanelProps> = ({ as = "div", allowCreate = false, readonly = false, alwaysShowEditPanel = false, ...props }) => {
    ...
    return (
    <MTagPanel<Tag> as={as} creatable={allowCreate} readonly={readonly} alwaysShowEditPanel={alwaysShowEditPanel} {...props} selectedItems={props.selectedItems} items={props.items} labelField={(t) => t.name} valueField={(t) => t.id?.toString()} colourField={(t: Tag) => t.colour as string} />
);
};
