...
public class TransactionRepository : RepositoryBase<Transaction, Guid>, ITransactionRepository
{
    ...
    protected override IQueryable<Transaction> GetById(Guid id) =>
        DataSet.Include(t => t.Splits).ThenInclude(t => t.Tags).Include(t => t.Splits).ThenInclude(t => t.OffsetBy).ThenInclude(to => to.OffsetByTransaction).Include(t => t.OffsetFor).ThenInclude(to => to.TransactionSplit).ThenInclude(ts => ts.Transaction).Where(t => t.TransactionId == id);
