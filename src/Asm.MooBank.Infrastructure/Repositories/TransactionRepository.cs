...
public class TransactionRepository : RepositoryBase<Transaction, Guid>, ITransactionRepository
{
    ...
    protected override IQueryable<Transaction> GetById(Guid id) =>
        DataSet.Include(t => t.TransactionSplits).ThenInclude(t => t.Tags).Include(t => t.OffsetBy).ThenInclude(to => to.OffsetByTransaction).Include(t => t.Offsets).ThenInclude(to => to.Transaction).Where(t => t.TransactionId == id);

    private IQueryable<Transaction> GetTransactionsQuery(Guid accountId) =>
        DataSet.Include(t => t.TransactionSplits).ThenInclude(t => t.Tags).Where(t => t.AccountId == accountId);
}